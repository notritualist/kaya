...
│        │   ├── __init__.py
│        │   ├── affective_analyzer.py   # Модуль аффективного преднанализа диалога
│        │   ├── baseline_manager.py     # Управление baseline: инициализация, OU-дрейф, эффекты выключений
│        │   ├── momentary_manager.py    # Управление моментальными срезами: создание, затухание, осаждение
│        │   ├── state_classifier.py     # Классификатор состояний: косинусное сходство с прототипами self_knowledge
...
│        │   ├── phs_scheduler.py        # Планировщик фоновых задач PHS (ежечасный дрейф, затухание momentary)
│        │   └── phs_cache.py            # Глобальный кэш экземпляров менеджеров PHS (оптимизация инициализации)
│        │
