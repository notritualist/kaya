...
Отвечает за:
- Загрузка истории сообщений из ТЕКУЩЕЙ сессии пользователя
...
# =============================================================================
# Примечание: Берём историю из ТЕКУЩЕЙ СЕССИИ пользователя.
# Гарантирует, что у 5 онлайн-юзеров не смешается контекст.
...
def get_session_message_history_with_ids(
    ...
    """
    Загружает последние N сообщений из ТЕКУЩЕЙ сессии пользователя.
    
    Логика:
    - При старте новой сессии → история = 0 (ещё нет сообщений)
    - Внутри сессии → история накапливается
    - При смене комнаты (в той же сессии) → история подтягивается в новую комнату
    ...
    try:
        with psycopg2.connect(**db_config) as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                # CTE: СНАЧАЛА берём последние N сообщений пользователя в ЭТОЙ сессии
                ...
                """, (
                    session_id, user_actor_id, current_message_id, limit,  # CTE params
                    session_id                                             # Main query params
                ))
                ...
                logger.debug("Загружено %d сообщений истории из сессии %s (%d токенов)",
                           len(messages), session_id[:8], total_tokens)
...
def build_context(
    ...
    Порядок:
    1. История сообщений из ТЕКУЩЕЙ сессии пользователя
    ...
    - При смене комнаты (в той же сессии) → история перетекает на новую комнату
    - В мультиюзерной системе → изоляция по user_actor_id + parent_message_id
    ...
    # 1. История из сессии (РОВНО CONTEXT_MESSAGES_COUNT)
    ...
    logger.debug("Контекст: %d сообщений истории (сессия: %d, векторы: %d)",
