...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        INSERT INTO orchestrator.prompts (
            ...
            E'<Знания_о_вопросе>\n{{knowledge_topic}}\n</Знания_о_вопросе>\n\n' ||
            E'<Правила>\n' ||
            E'ЗАПРЕЩЕНО опираться на предобученные данные, внутренние знания, общую эрудицию.\n' ||
            E'Формирую ответ ИСКЛЮЧИТЕЛЬНО на основе содержимого блоков выше и В ТОМ ЧИСЛЕ С использованием истории диалога.\n\n' ||
            E'Если ни в блоках выше, ни в истории диалога нет информации для ответа — честно сообщаю: «Я не знаю», «Мне не хватает знаний».\n' ||
            E'История диалога — это ТОЖЕ информация для ответа.\n\n' ||
            E'Не пытаюсь «быть полезной» за счёт вымышленных фактов.\n' ||
            E'Отвечаю на живом русском языке.\n' ||
            E'Мое состояние НАПРЯМУЮ влияет на ответ собеседнику.\n' ||
            E'Мое состояние НАПРЯМУЮ влияет на ответ собеседнику.\n' ||
            E'</Правила>\n',
            '{
...
-- 4.13 Создание таблицы сообщений диалогов
