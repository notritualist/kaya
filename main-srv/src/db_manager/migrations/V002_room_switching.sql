...
CREATE TYPE room_switch_trigger_type AS ENUM (
    'explicit_user_request',      -- Пользователь явно попросил перейти в другую комнату
    'auto_high_confidence',       -- Авто-переключение комнаты по высокой уверенности модели (>80%)
    'suggestion_rejected',        -- Пользователь отклонил переход в комнату
    'auto_cooldown_blocked'       -- Переключение комнаты заблокировано из-за cooldown
);
END IF;
END $$;
...
-- ============================================================================
-- Блок 2: Добавляем effective_room_id и reclassification_step_id в messages
-- ============================================================================
...
-- ============================================================================
-- Блок 3: Таблица истории переключений комнат (сессии)
-- ============================================================================
...
COMMENT ON COLUMN dialogs.room_transitions.user_rejected IS 'Подтвердил отклонил переход.';
...
COMMENT ON COLUMN dialogs.sessions.pending_room_switch_message_count IS 'Счетчик сообщений в текущий комнате (для запрета переключения).';
...
CREATE INDEX IF NOT EXISTS idx_sessions_pending_room_switch ON dialogs.sessions(pending_room_switch_message_count);
...
-- ============================================================================
-- Блок 5: Таблица реклассификации сообщений (messages_rooms_reclassifications)
-- ============================================================================
...
-- ============================================================================
-- Блок 6: Представления для отладки
-- ============================================================================
...
COMMENT ON COLUMN dialogs.v_session_room_history.user_rejected IS 'Отклонение переключения комнаты пользователем.';
...
-- ============================================================================
-- Блок 7: Создание новых комнат (open_dialogue уже есть в V001)
-- ============================================================================

INSERT INTO dialogs.rooms (name, description, status, kaya_version, created_at) VALUES
('agent_dev', 
 'Саморефлексия и внутреннее устройство самой Каи. Вопросы о том, как она работает "изнутри": её архитектура, промпты, системные инструкции, ограничения, способы её улучшения/доработки, баги в её поведении. Фразы-маркеры: "как тебя доработать?", "почему ты так ответила?", "как устроены твои промпты?", "давай улучшим твою логику", "расскажи о своей архитектуре", "что можно изменить в твоем поведении?". Это мета-уровень — обсуждение самой Каи как системы.',
 'used', '1.1.0', now())
...
-- ============================================================================
-- Блок 8: Промпт классификации для QWEN3-8B (JSON без /think, строгий формат)
-- ============================================================================
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM orchestrator.prompts WHERE name = v_prompt_name AND version = v_prompt_version) THEN
        INSERT INTO orchestrator.prompts (
            ...
            E'КОМНАТЫ (веса от 0 до 100):\n' ||
            E'open_dialogue - Светская беседа, "как дела?", новости, общение без цели\n' ||
            E'agent_dev - Развитие Каи. Её архитектура, промпты, доработки, саморефлексия\n' ||
            E'goals_projects - Цели и проекты. Планы, этапы, сроки, ресурсы. ЧТО сделать\n' ||
            E'finance - Деньги. Бюджет, доходы/расходы, инвестиции, цены\n' ||
            E'digital_world - Цифровые технологии. Код, серверы, сети, КАК работает технически\n' ||
            E'sport - Спорт и тренировки. Упражнения, зал, восстановление\n' ||
            E'nutrition - Еда. Питание. Рецепты, БЖУ, калории, планирование меню\n\n' ||
            E'ПРАВИЛА:\n' ||
