...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM orchestrator.prompts WHERE name = v_prompt_name AND version = v_prompt_version) THEN
        INSERT INTO orchestrator.prompts (
            ...
            E'3. Определи, есть ли ПРЯМОЙ запрос на смену комнаты (фразы: "давай про...", "перейдём к...", "поговорим о...", "сменим тему диалога...").\n' ||
            E'4. Выбери комнату из ДОСТУПНЫЕ КОМНАТЫ.\n\n' ||
            E'ПРАВИЛА:\n' ||
            E'1. room_weights: общая сумма весов комнат = 100. Распредели веса между комнатами по значимости.\n' ||
            ...
            E'4. Если общей темы диалога нет в ДОСТУПНЫХ КОМНАТАХ выбирай "open_dialogue"\n' ||           
