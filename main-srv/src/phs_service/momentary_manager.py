...
Features:
- Creates momentary slices from baseline at session start.
- Tracks origin: session_id, baseline_id, actor_id, state_id.
- Sediments momentary experience back to baseline (session end, hourly, crash).
- Handles decay via orchestrator task phs_momentary_decay.
- Decay formula: new = baseline + (momentary - baseline) * (1 - alpha) + noise.
- Manages dangling momentary cleanup on startup/crash recovery.
- All changes create NEW rows (immutable history), old rows deactivated.
- Full traceability: output_data contains IDs before/after for all operations.

Architecture:
- Momentary created per physical session, shared across dialogues.
- Sedimentation applies alpha coefficient to merge momentary into baseline.
- Decay task scheduled by PHSScheduler, handled via handle_decay_task.
- Crash recovery sediments all active momentaries before resetting flags.
"""

version = "1.1.0"
description = "Momentary state manager"
...
class MomentaryManager:
    ...
    def _compute_dynamic_alpha(self) -> float:
        """
        Вычисляет динамический коэффициент осаждения: 1.0 / N_активных_momentary пользователей.
        
        Для многопользовательского агента: каждый пользователь вносит 
        равный вклад в общий baseline. Если активна 1 сессия → alpha=1.0,
        если 5 сессий → alpha=0.2.
        
        Returns:
            float: Коэффициент осаждения [0..1].
        """
        with psycopg2.connect(**self.db_config) as conn:
            with conn.cursor() as cur:
                cur.execute("SELECT COUNT(*) FROM dialogs.sessions WHERE status = 'active'")
                row = cur.fetchone()
    ...
    def sediment_momentary_to_baseline(
        ...
        """
        Осаждает momentary в baseline.
        
        Коэффициент alpha вычисляется динамически: 1.0 / N_активных_сессий.
        Формула: new_value = old_baseline + alpha * (momentary - old_baseline).
        Создаёт НОВУЮ запись baseline, старая деактивируется.
        ...
        Returns:
            dict | None: Словарь с трассировкой {baseline_id_before, baseline_id_after,
                        momentary_id, actor_id, alpha} или None, если нет данных.
        """
        # Динамический расчёт alpha
    ...
    def sediment_all_active_momentaries(
        ...
        Коэффициент alpha вычисляется динамически для каждого актора в sediment_momentary_to_baseline.
        ...
        for actor_id in actors:
            result = self.sediment_momentary_to_baseline(actor_id, reason_code)
            if result:
                count += 1
    ...
    def apply_decay_tick(self, step_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        Логика:
        1. Для каждого активного momentary вычисляет новые значения по формуле затухания.
        2. Создаёт НОВУЮ запись momentary с обновлёнными гормонами, вектором и state_id.
        3. Деактивирует предыдущую запись (is_active = FALSE).
        4. Возвращает список созданных записей для трассировки.
        
        Формула с коэффициентом и шумом:
            new_val = baseline + (momentary - baseline) * (1 - alpha) + noise * gauss(0, 1)
        
        Коэффициент alpha читается из state.settings (alpha_momentary_decay).
        Шум добавляет стохастические микрофлуктуации.
        
        Returns:
            dict: Результат с updates списком, содержащим momentary_id_before/after.
        """
        # Загружаем alpha и noise
        alpha = self._compute_dynamic_alpha()
        
        # Шум оставляем из настроек
        with psycopg2.connect(**self.db_config) as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                cur.execute("SELECT value_float FROM state.settings WHERE param_name = 'momentary_drift_noise'")
                row = cur.fetchone()
                noise_scale = float(row['value_float']) if row and row.get('value_float') is not None else 0.5
        
        # Получаем активные momentary с baseline_id
        with psycopg2.connect(**self.db_config) as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                cur.execute(
                    """
        ...
        for m in momentaries:
            ...
            # Новые значения
            new_cort = m['b_cort'] + (m['cortisol'] - m['b_cort']) * decay_factor + noise_cort
            new_dopa = m['b_dopa'] + (m['dopamine'] - m['b_dopa']) * decay_factor + noise_dopa
            new_oxy = m['b_oxy'] + (m['oxytocin'] - m['b_oxy']) * decay_factor + noise_oxy
            ...
            with psycopg2.connect(**self.db_config) as conn:
                with conn.cursor() as cur:
                    ...
                    cur.execute("""
                        ...
                        RETURNING id
                    """, (
                        m['session_id'], m['baseline_id'], m['actor_id'], dialog_id,  # ← dialog_id добавлен
